...
/* AlertDialog — sharp, ink-bordered confirmation modal. Controlled: pass
   ...
   approach) — `.sw-alert-overlay` / `.sw-alert-content` in globals.css fade +
...
export function V3AlertDialog({
  ...
  return (
    <AlertDialog.Root open={open} onOpenChange={onOpenChange}>
      <AlertDialog.Portal>
        <AlertDialog.Overlay className="sw-alert-overlay fixed inset-0 z-[60] bg-overlay [backdrop-filter:blur(6px)_saturate(1.05)] [-webkit-backdrop-filter:blur(6px)_saturate(1.05)]" />
        <AlertDialog.Content
          className={cn(
            'sw-alert-content fixed top-1/2 left-1/2 z-[70] w-[calc(100vw-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 border border-ink bg-bg text-ink shadow-drawer outline-none',
          )}
        >
          <div className="border-b border-ink px-5 py-3">
            <AlertDialog.Title className="sw-eyebrow m-0 text-[11px]">
              {title}
            </AlertDialog.Title>
          </div>
          ...
          <div className="flex justify-end gap-2 border-t border-ink px-5 py-3">
            <AlertDialog.Cancel className="sw-eyebrow sw-focus cursor-pointer border border-ink bg-transparent px-3.5 py-1.5 text-[10px] text-ink">
              {cancelLabel}
            </AlertDialog.Cancel>
            <AlertDialog.Action
              onClick={onConfirm}
              className={cn(
                'sw-eyebrow sw-focus cursor-pointer border-0 px-3.5 py-1.5 text-[10px] text-white',
