...
/* Modal — centered, ink-bordered dialog in the admin Syncwave style.
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal container={container}>
        <Dialog.Overlay className="sw-drawer-overlay fixed inset-0 z-40 bg-overlay [backdrop-filter:blur(8px)_saturate(1.1)] [-webkit-backdrop-filter:blur(8px)_saturate(1.1)]" />
        <Dialog.Content
          ref={contentRef}
          aria-describedby={undefined}
          className={cn(
            'sw-modal-pop fixed top-1/2 left-1/2 z-50 flex max-h-[calc(100vh-3rem)] w-[var(--modal-w,600px)] max-w-[calc(100vw-2rem)] flex-col border border-ink bg-[var(--card-bg,var(--bg))] text-ink shadow-drawer outline-none',
        ...
        >
          <div className="flex items-baseline justify-between gap-3 border-b border-ink px-5 py-3">
            ...
            <Dialog.Close
              className="sw-focus cursor-pointer border-0 bg-transparent p-0 text-[22px] leading-none text-muted"
              aria-label="Close"
            >
              ×
            </Dialog.Close>
          ...
          <div className="sw-scroll flex-1 overflow-auto px-5 py-4">{children}</div>
