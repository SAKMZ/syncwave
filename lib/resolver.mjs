  ...
  if (/sign in to confirm you.?re not a bot|cookies for the authentication/i.test(text)) {
    return new Error(
      "YouTube bot-check. On a home connection this usually means a temporary " +
        ...
        "itself is blocked: upload a cookies.txt in /admin, or set YTDLP_PROXY. " +
        "See DEPLOY.md."
    );
  }
...
function runDownload(out, videoId, onProgress, attempt = 1) {
  return new Promise((resolve, reject) => {
    ...
    sub.then(() => resolve(out)).catch((err) => {
      // A bot-check won't pass on an immediate retry — fail fast so the room
      // skips sooner. (A 429 won't clear in 1.2s either.)
      const fatal =
        /sign in to confirm you.?re not a bot/i.test(errTail) ||
        /HTTP Error 429|Too Many Requests/i.test(errTail);
      if (attempt < 2 && !fatal) {
        setTimeout(
