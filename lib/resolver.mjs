...
// Hard ceiling on cached audio. Age alone can't bound the cache — a busy room
// pulls more inside 72h than a small disk holds — and this usually runs on a
// machine somebody also uses. 0 disables the cap for anyone who'd rather
// manage the disk themselves.
const CACHE_MAX_BYTES = Math.max(0, Number(process.env.CACHE_MAX_MB ?? 4096)) * 1024 * 1024;
...
/**
 * Which audio track to ask YouTube for.
 *
 * Preferring m4a matters more than it looks. YouTube's best audio is Opus in a
 * webm container, and converting that to m4a re-encodes it — which both loses a
 * generation of quality and *inflates* the file, since the encoder is told to
 * aim high: a 4.4 MB Opus stream came back out as a 14 MB m4a. Asking for the
 * m4a rendition instead means ffmpeg only rewrites the container, so the file
 * stays 4.3 MB and the audio is untouched.
 *
 * On a proxy the bitrate is capped as well, because that traffic is usually
 * metered — dropping to <=96 kbps takes the same track to 1.6 MB. Measured
 * against a live instance that works out at ~2.7 MB of billed traffic per
 * track rather than ~4.8 MB, once metadata requests and protocol overhead are
 * counted: a 1 GB allowance goes from ~210 tracks to ~380. Direct connections
 * are never capped; there is nothing to save.
 */
function audioFormat(viaProxy) {
  const cap = Number(process.env.PROXY_MAX_ABR ?? 96);
  if (viaProxy && cap > 0) {
    return (
      `bestaudio[ext=m4a][abr<=${cap}]/bestaudio[abr<=${cap}]/` +
...
function runDownload(out, videoId, onProgress, proxy = "", attempt = 1) {
  return new Promise((resolve, reject) => {
    ...
    const sub = youtubedl.exec(`https://music.youtube.com/watch?v=${videoId}`, {
      output: out,
      format: audioFormat(Boolean(PROXY)),
      extractAudio: true,
      audioFormat: "m4a",
      noPlaylist: true,
      noWarnings: true,
...
/**
 * How many tracks may be fetched at once.
 *
 * Each download is a yt-dlp process and an ffmpeg remux, so this is the one
 * place where a busy instance turns CPU-bound. `inFlight` already collapses
 * duplicate requests for the same track, which covers a room where everyone
 * queues the same song — but not a public instance where twenty rooms each
 * queue something different. Unbounded, that is twenty processes on however
 * many cores the box has, and every one of them finishes slower than three
 * would have.
 *
 * The ceiling doesn't lose any work: waiting jobs start the moment a slot frees
 * up, and playback only ever needs the *current* track ready — the rest are
 * warming ahead of time and can afford to wait their turn.
 *
 * 0 disables the limit, for a machine that would rather saturate itself.
 */
const MAX_PARALLEL = Math.max(0, Number(process.env.MAX_PARALLEL_DOWNLOADS ?? 3));
let active = 0;
const waiting = [];

function acquireSlot() {
  if (MAX_PARALLEL === 0 || active < MAX_PARALLEL) {
    active++;
...
function releaseSlot() {
  ...
  // so a burst can't slip past the ceiling in the gap between the two.
  if (next) next();
  else active--;
}
...
export async function ensureAudio(videoId, onProgress) {
  ...
  const job = (async () => {
    await acquireSlot();
    // The file may have arrived while this job sat in the queue — another room
    // asking for the same track, or the cache being warmed from elsewhere.
    if (fileExistsSync(out)) return out;
    try {
      return await downloadWithFallback(out, videoId, onProgress);
    } finally {
      releaseSlot();
    }
  })().finally(() => {
...
/** Every cached track with its size and last play, newest first. */
async function cacheEntries() {
  ...
  for (const f of files) {
    ...
    const last = cacheIndex[id]?.lastPlayed ?? mtime;
    out.push({ id, full, size, last, meta: cacheIndex[id] });
  }
  return out.sort((a, b) => b.last - a.last);
}

/** What the cache is using right now, for the admin console. */
export async function cacheStats() {
  ...
  return {
    ...
    retentionHours: Math.round(RETAIN_MS / 3600000),
    oldestPlayed: entries.length ? entries[entries.length - 1].last : null,
  };
}

/** Delete every cached track. Rooms are unaffected; tracks re-download on play. */
export async function purgeCache() {
  const entries = await cacheEntries();
  for (const e of entries) {
    await fs.unlink(e.full).catch(() => {});
    delete cacheIndex[e.id];
  }
  saveDebounced("cache-index", cacheIndex);
  return { removed: entries.length, freed: entries.reduce((n, e) => n + e.size, 0) };
}

/**
 * Evict cached audio, by age and then by size.
 *
 * Age alone is not a bound. A busy room can pull far more than the disk holds
 * inside the retention window, and this usually runs on a machine somebody
 * also lives on — filling their disk is a worse failure than re-downloading a
 * track. So once the age pass is done, the least recently played files go too
 * until the total is back under the cap.
 */
export async function sweepCache() {
  try {
    const now = Date.now();
    let entries = await cacheEntries();

    for (const e of entries) {
      if (now - e.last > RETAIN_MS) {
        await fs.unlink(e.full).catch(() => {});
        delete cacheIndex[e.id];
        e.evicted = true;
      }
    }
    entries = entries.filter((e) => !e.evicted);

    let total = entries.reduce((n, e) => n + e.size, 0);
    if (CACHE_MAX_BYTES > 0 && total > CACHE_MAX_BYTES) {
      const before = total;
      // Oldest last: pop from the end until we're under.
      for (let i = entries.length - 1; i >= 0 && total > CACHE_MAX_BYTES; i--) {
        await fs.unlink(entries[i].full).catch(() => {});
        delete cacheIndex[entries[i].id];
        total -= entries[i].size;
      }
      console.log(
        `[cache] over limit: trimmed ${mb(before - total)} MB, now ${mb(total)}/${mb(CACHE_MAX_BYTES)} MB`
      );
    }
    saveDebounced("cache-index", cacheIndex);
  } catch (e) {
