...
function describeFailure(err, stderr) {
  ...
  if (/sign in to confirm you.?re not a bot|cookies for the authentication/i.test(text)) {
    return new Error(
      "YouTube bot-check: this host's IP has to supply a logged-in session. " +
        "Export a cookies.txt and set YTDLP_COOKIES_FILE — see DEPLOY.md. " +
        "This is common on cloud/VPS IPs and expected on Railway/Render."
    );
  }
